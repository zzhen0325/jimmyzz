...
export default function ChromeScene({ workProgress, interactive }: { workProgress: RefObject<number>; interactive: boolean }) {
  ...
  return <>
    <canvas data-hover-label={interactive ? "scroll" : undefined} ref={canvas} tabIndex={interactive ? 0 : -1} role="img" aria-label="银色 ZZ、漂浮的黑色高光 Jimmy 手写字标、黄色和灰色立体角色、蓝色凹刻 ZZ 挂件、笑脸键帽、始终朝向镜头的滑板人物和黄绿色像素漩涡、五个立牌、游戏手柄的物理互动场景。按住拖动，松开弹散；方向键环绕，空格弹散。点击绿色漩涡或按 V 触发吸入与喷出彩蛋，Esc 取消。" aria-describedby="chrome-interaction-hint" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", touchAction: "none", cursor: "grab" }} />

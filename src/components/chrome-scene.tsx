...
export default function ChromeScene({ workProgress, interactive }: { workProgress: RefObject<number>; interactive: boolean }) {
  ...
  useEffect(() => {
    ...
    try {
      controller.current = createChromeWorld(canvas.current, { workProgress, debug: process.env.NODE_ENV !== "production", onError: fail });
    } catch { fail(); }
    return () => { mounted = false; controller.current?.dispose(); controller.current = null; };
  }, [workProgress]);
  return <>
    <canvas data-hover-label={interactive ? "scroll" : undefined} ref={canvas} tabIndex={interactive ? 0 : -1} role="img" aria-label="银色 ZZ、五个立牌、游戏手柄与几何物件的物理互动场景。按住拖动，松开弹散；方向键环绕，空格弹散。" aria-describedby="chrome-interaction-hint" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", touchAction: "none", cursor: "grab" }} />
